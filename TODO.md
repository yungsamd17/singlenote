...
- [ ] Soften "fully offline" to "offline-first" everywhere (README, About,
...
- [ ] Backup rules: `allowBackup=true` with no extraction rules
...
- [ ] Lockscreen leak: pinned notification is `VISIBILITY_PUBLIC` with full
...
- [ ] Download action: `GithubReleases.kt:72` matches an asset literally named
...
- [ ] `THIRD_PARTY_LICENSES` omits Room/Compose/Glance/DataStore/Navigation/
...
- [ ] `runBlocking` DataStore read on the UI thread (`MainActivity.kt:194-195`,
  every cold start and resume). Use the SplashScreen API or an async theme gate.
- [ ] Fixed-height no-scroll editor clips data (`ui/NoteScreen.kt:227-229,689-691`).
  At large font scales pasted content is unreachable and silently truncated.
  Cap length if needed, but let the field scroll.
- [ ] Restore-conflict dialog crams Cancel/Swap/Replace into `confirmButton`
...
- [ ] Non-atomic writes: `saveActive`/`swapWithActive` are read-modify-write
...
- [ ] Dependencies ~9 months stale (Kotlin 2.1.0, AGP 8.7.3, BOM 2024.12.01),
  no Dependabot/Renovate. The 2.1→2.2 + AGP 8→9 jump breaks KSP all at once.

## Accessibility and UX

- [ ] `selectable` abused as `clickable` on Settings/About rows
...
- [ ] Empty-note archive/delete FABs are alpha-only, still focusable and
  tappable (`ui/NoteScreen.kt:540-575`). Set `enabled=hasContent`.
- [ ] Radio rows miss 48dp and `RadioButton(onClick=null)` isn't focusable
  (`SettingsScreen.kt:340-345`). 48dp rows, `Role.RadioButton`, `selectableGroup`.
- [ ] Editor has no accessible label: the hint overlay is invisible to TalkBack
  (`ui/NoteScreen.kt:693-825`). Add `semantics { hint = … }`.
- [ ] No Undo anywhere — archive/delete/clear are irreversible with
  informational-only snackbars.
- [ ] Archive snackbar ignores insets (`ArchiveScreen.kt:107`) and sits under
  the gesture bar; match the note screen's lifted host.
- [ ] Changelog error state is a dead end (`ui/AboutScreen.kt:281-285`) — add
  a Retry button instead of dismiss-and-reopen.
- [ ] Blank first frame while prefs load (`NoteScreen.kt:432-435`,
  `SettingsScreen.kt:98-101`) — spinner with a "Loading" label for TalkBack.
- [ ] Widget: no preview/description, 2x1 cell rendering 6 lines that always
  ellipsize, hardcoded English fallback (`SinglenoteWidget.kt:66`) instead of
  `strings.xml`.
- [ ] Tile subtitle has no TalkBack distinction between label and subtitle
  (`tile/NoteTileService.kt:37`).

## Hardening nits

- [ ] No tapjacking defense (`filterTouchesWhenObscured`) on one-tap
  Archive/Delete/Pin.
- [ ] Plaintext SQLite + prefs, no encryption story — fine if disclosed next
  to the "privacy first" claim, not fine silent.
- [ ] Copy isn't `EXTRA_IS_SENSITIVE` (API 33+) and lingers in keyboard history
  (`ui/NoteScreen.kt:250-251`); Share is one-tap `text/plain` to any target.
- [ ] `singleTask` with no `onNewIntent`; `BOOT_COMPLETED` without
  `MY_PACKAGE_REPLACED` (pin vanishes on every update); no explicit
  `usesCleartextTraffic=false` while `http://` links render.
- [ ] `portrait` lock + `adjustNothing` bans foldables, tablets, landscape and
  multi-window (`AndroidManifest.xml:23-24`).
- [ ] Deprecated `URL()` + raw `HttpURLConnection` (`GithubReleases.kt:27`);
...
- [ ] Issue template asks for "browser" on a native app, no version/device/
  logcat fields; PR template placeholder fits bots, not humans.
- [ ] Docs-only CI skip forgets extensionless `THIRD_PARTY_LICENSES`
  (`build.yml:34-35`).

## Docs drift (recurring)

- [ ] README + plan version references rot on every release (`v0.3.3` named
